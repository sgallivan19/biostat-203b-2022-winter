...
```{r}
# change thirty day mortality entries to binary instead of string 
df_train_logreg <- df_train %>% 
  mutate(
    thirty_day_mort = ifelse(thirty_day_mort == "Yes", 1, 0)
  )
...
# log reg model 
logreg <- glm(thirty_day_mort ~ . -thirty_day_mort, 
                data = df_train_logreg, family = binomial())
# display results 
summary(logreg)
```
...
```{r}
# model 
logreg_test <- glm(thirty_day_mort ~ . -thirty_day_mort, 
                data = df_test_logreg, family = binomial())
# predicted values
log_prob <- predict(logreg_test, type = "response")
#translate probabilities to predictions
log_predict <- ifelse(log_prob > .5, 1, 0)
#confusion matrix
table(predicted = log_predict, observed = df_test_logreg$thirty_day_mort)
# accuracy 
cat("Test Accuracy:", mean(log_predict == df_test_logreg$thirty_day_mort))
```
