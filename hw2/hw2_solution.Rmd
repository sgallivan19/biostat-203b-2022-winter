...
```{r}
admissions_tble1 <- semi_join(admissions_tble, icustays_tble, 
...
Admission month, monthday and weekly are all uniformly distributed. Interestingly enough, 
admission hour peaks at midnight, decreases until it spikes at 7am, jumps back down at 8am and 
gradually increases until 3pm where it stays constant until midnight.   

```{r}
#admission year  
c <- ggplot(admissions_tble1, aes(year(admittime)))
c + geom_bar()

#admission month
d <- ggplot(admissions_tble1, aes(lubridate::month(admittime, label = T)))
d + geom_bar() + labs(x = "Month")

#admission month day 
a <- ggplot(admissions_tble1, aes(lubridate::mday(admittime)))
a + geom_bar() + labs(x = "month day")

#admission week day 
b <- ggplot(admissions_tble1, aes(lubridate::wday(admittime, label = T)))
b + geom_bar() + labs(x = "weekday")

#admission hour
e <- ggplot(admissions_tble1, aes(lubridate::hour(admittime)))
...
patients_tble1 <- semi_join(patients_tble, icustays_tble1, by = "subject_id")
patients_tble1
```
...
```{r}
#summarize gender 
gender <- ggplot(patients_tble1, aes(gender))
gender + geom_bar()

#summarize anchor_age
age <- ggplot(patients_tble1, aes(anchor_age))
age + geom_histogram()
```
...
```{r}
read_csv("/mnt/mimiciv/1.0/hosp/d_labitems.csv.gz", col_types = cols_only(
  itemid = col_factor(c(
    "50912", "50971", "50983", "50902", "50882", "51221",
    "51301", "50931", "50960", "50893"))), lazy)
```
