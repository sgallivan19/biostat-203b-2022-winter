...
labevents_tble <- left_join(labevents_tble, d_labitems, by = "itemid")
```
...
```{r}
fivefoura <- ggplot(labevents_tble, aes(category))
fivefoura + geom_bar()

fivefourb <- ggplot(labevents_tble, aes(fluid))
fivefourb + geom_bar()

fivefourc <- ggplot(labevents_tble, aes(label))
fivefourc + geom_bar()
```
...
chartevents_tble
#label
sixa <- ggplot(chartevents_tble, aes(label))
sixa + geom_bar()
#category
sixb <- ggplot(chartevents_tble, aes(category))
sixb + geom_bar()
```
...
mutate(age_hadm = anchor_age +year(admittime) -anchor_year) %>% 
  filter(age_hadm > 18) %>% 
  mutate(thirty_day_mort = ifelse(is.na(deathtime), Inf, deathtime) - admittime <= 30)
```
